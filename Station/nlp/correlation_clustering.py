...
def clustering(input_sentence, model_path):
    model = Logger(model_path).model

    tokenizer = TokenizerLogger().tokenizer
    nouns = tokenizer.nouns(input_sentence)
    print(nouns)
    ...
    input_words.extend([m[0] for m in related_words[:topn_num]])
    input_correlation.extend([m[2] for m in related_words[:topn_num]])
